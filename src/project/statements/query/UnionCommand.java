 ...
 */
public class UnionCommand implements ISetCommand {

	private Pattern pattern = Pattern.compile("union +(.+?) +and +(.+?) *;", Pattern.CASE_INSENSITIVE);
	...
	public DataSet execute(DataManipulator d1, DataManipulator d2) throws HardyException {
		if(d2 == null) {
			if(d1 == null) {
				return Database.get().union(query1, query2);
			}
			if(query1.contains("$")) {
				return Database.get().union(d1, query2, false);
			} return Database.get().union(d1, query1, true);
		}
		return Database.get().union(d1, d2);
	}
