 ...
 */
public class SymmetricDifferenceCommand implements ISetCommand {

	private Pattern pattern = Pattern.compile("symmetric difference +(.+?) +and +(.+?) *;", Pattern.CASE_INSENSITIVE);
	...
	public DataSet execute(DataManipulator d1, DataManipulator d2) throws HardyException {
		if(d2 == null) {
			if(d1 == null) {
				return Database.get().symmetricDifference(query1, query2);
			}
			if(query1.contains("$")) {
				return Database.get().symmetricDifference(d1, query2, false);
			} return Database.get().symmetricDifference(d1, query1, true);
		}
		return Database.get().symmetricDifference(d1, d2);	}
